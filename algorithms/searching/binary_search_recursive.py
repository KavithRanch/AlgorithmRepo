...
def binary_search_recursive(array, target):  # Pre-condition: Array must be sorted (ascending)
    if len(array) > 0:  # Checking whether our range includes at least one value
        mid_value = (0 + len(array)) // 2  # Middle value will be halfway point between upper and lower
        ...
        if target == array[mid_value]:  # If the target value is equal to the middle element, return the index mid_value
            ...
            return binary_search_recursive(array[mid_value + 1:], target)
